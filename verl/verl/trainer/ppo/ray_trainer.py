...
from o2searcher.generation import LLMGenerationManager, GenerationConfig
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            n_val_samples = self.config.actor_rollout_ref.rollout.n_agent_val
            test_batch = test_batch.repeat(repeat_times=n_val_samples, interleave=True)
            test_gen_batch = test_batch.pop(['input_ids', 'attention_mask', 'position_ids'], non_tensor_batch_keys=['ability'])
            ...
            with _timer('step', timing_raw):
                ...
                search_stats_list.append(np.array(test_batch.meta_info['valid_search_stats'], dtype=np.int16))
                action_stats_list.append(np.array(test_batch.meta_info['valid_action_stats'], dtype=np.int16))
                query_stats_list.append(np.array([len([r for r in q.split('\n') if r.strip()]) for q in test_batch.meta_info['queries']], dtype=np.int16))
                reward_tensor_lst.append(reward_tensor)
                data_source_lst.append(test_batch.non_tensor_batch.get('data_source', ['unknown'] * reward_tensor.shape[0]))
            
        # reward_tensor = torch.cat(reward_tensor_lst, dim=0).sum(-1).cpu()  # (batch_size,) # TODO: a little bit different with search-r1, need to check again
        reward_tensor = torch.cat([rw.sum(-1) for rw in reward_tensor_lst], dim=0).cpu()
        search_tensor = np.concatenate(search_stats_list, axis=0)
        action_tensor = np.concatenate(action_stats_list, axis=0)
        query_tensor = np.concatenate(query_stats_list, axis=0)
        ...
        for i in range(reward_tensor.shape[0]):
            ...
            if data_source not in data_source_reward:
                ...
                data_source_search[data_source] = []
                data_source_action[data_source] = []
                data_source_query[data_source] = []
            data_source_reward[data_source].append(reward_tensor[i].item())
            data_source_search[data_source].append(search_tensor[i].item())
            data_source_action[data_source].append(action_tensor[i].item())
            data_source_query[data_source].append(query_tensor[i].item())

        metric_dict = {}
        for data_source, rewards in data_source_reward.items():
            metric_dict[f'val/test_score/{data_source}'] = np.mean(rewards)
            metric_dict[f'val/search_stats/{data_source}'] = np.mean(data_source_search[data_source])
            metric_dict[f'val/action_stats/{data_source}'] = np.mean(data_source_action[data_source])
            metric_dict[f'val/query_stats/{data_source}'] = np.mean(data_source_query[data_source])
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                gen_batch = batch.pop(batch_keys=['input_ids', 'attention_mask', 'position_ids'], non_tensor_batch_keys=['ability'])
                ...
                with _timer('step', timing_raw):
                    ...
                    with torch.no_grad():
                        old_log_prob = self.actor_rollout_wg.compute_log_prob(final_gen_batch_output)
                        final_gen_batch_output = final_gen_batch_output.union(old_log_prob)
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with _timer('update_actor', timing_raw):
                            # mask extra info from searching
